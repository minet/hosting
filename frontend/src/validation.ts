...
export function validateVmName(v: string): string | null {
  ...
  if (v.length > 60) return 'Le nom ne doit pas dépasser 60 caractères'
  if (!VM_NAME_RE.test(v)) return 'Caractères autorisés : lettres, chiffres et tirets (doit commencer par alphanumérique)'
  return null
}
