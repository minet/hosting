...
export default function VMInfoCard({ vm, status, loadingAction, running, isOwner, uptime, onOpenDnsRequest, onOpenIpRequest }: Props) {
  ...
  return (
    <div className="md:col-span-2 xl:col-span-2 border border-neutral-100 dark:border-neutral-800 shadow-md dark:shadow-none rounded-sm bg-white dark:bg-neutral-900 px-4 2xl:px-4 py-2.5 2xl:py-3 flex flex-col gap-2 2xl:gap-4 min-w-0 overflow-hidden">
      <div className="flex items-center gap-2 2xl:gap-4 pb-1.5 2xl:pb-3 border-b border-neutral-100 dark:border-neutral-800 min-w-0">
        {loadingAction ? (
          <span className="w-2.5 h-2.5 2xl:w-3.5 2xl:h-3.5 shrink-0 rounded-full border-2 border-neutral-300 dark:border-neutral-600 border-t-neutral-600 dark:border-t-neutral-300 animate-spin" />
        ) : (
        ...
        {isOwner ? (
          ...
          <Tooltip tip={t('info.sharedVM')} className="shrink-0">
            <Share2 size={is2xl ? 26 : 13} className="text-blue-400" />
          </Tooltip>
        )}
      ...
      <div className="grid grid-cols-2 gap-x-4 2xl:gap-x-8 gap-y-1 2xl:gap-y-3 w-full flex-1 min-w-0 content-start 2xl:content-center">
