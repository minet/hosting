    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <form onSubmit={handleAdd} className="flex items-end gap-3 shrink-0">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-neutral-500 dark:text-neutral-400">ID Proxmox</label>
          <input
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 z-10 border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider border-r border-neutral-200 dark:border-neutral-700">Nom</th>
              <th className="px-3 py-2 text-center text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider w-24 border-r border-neutral-200 dark:border-neutral-700">Actif</th>
          ...
          <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-100 dark:divide-neutral-800">
            {loading && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-neutral-400 dark:text-neutral-500 text-xs"><Loader size={14} className="animate-spin inline mr-2" />Chargement…</td></tr>
            )}
            {!loading && templates.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-neutral-400 dark:text-neutral-500 text-xs">Aucun template</td></tr>
            )}
            {templates.map(t => (
              <tr key={t.template_id} className={`hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors ${!t.is_active ? 'opacity-50' : ''}`}>
                <td className="px-3 py-2 font-mono text-xs text-neutral-500 dark:text-neutral-400 border-r border-neutral-100 dark:border-neutral-800">{t.template_id}</td>
                <td className="px-3 py-2 text-neutral-700 dark:text-neutral-300 border-r border-neutral-100 dark:border-neutral-800">{t.name}</td>
                <td className="px-3 py-2 text-center border-r border-neutral-100 dark:border-neutral-800">
                  <button
                    onClick={() => handleToggleActive(t.template_id, t.is_active)}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors cursor-pointer ${t.is_active ? 'bg-emerald-500' : 'bg-neutral-300 dark:bg-neutral-600'}`}
                    title={t.is_active ? 'Désactiver' : 'Activer'}
                  >
                    <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${t.is_active ? 'translate-x-4' : 'translate-x-1'}`} />
                  </button>
                </td>
