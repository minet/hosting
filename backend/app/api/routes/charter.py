...
def _send_charter_email(
    *,
    ...
    ctx = {"base_url": base_url, "prenom": prenom, "nom": nom, "signed_at": signed_at, "signed_at_display": signed_at_display}
    html = _jinja_env.get_template("charter_signed.html.j2").render(ctx)
    plain = _jinja_env.get_template("charter_signed.txt.j2").render(ctx)
...
async def sign_charter(
    ...
    """
    claims = await current_user_claims(ctx.payload)
